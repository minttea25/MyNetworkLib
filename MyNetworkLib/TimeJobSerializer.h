...
/// </summary>
ABSTRACT class TimeJobSerializer : public enable_shared_from_this<TimeJobSerializer>
{
public:
	virtual ~TimeJobSerializer()
	{
		DESTRUCTOR(TimeJobSerializer);
	}
	...
	bool TryExecuteOneJob(const uint64 now)
	{
		TimeJobSPtr job = nullptr;

		if (_jobs.TryPop(OUT job, _check_condiion_time_job(now)))
		{
			job->Execute();
			return true;
		}
...
private:
	LockPriorityQueue<TimeJobSPtr, TimeJob::TimeJobSPtrComp> _jobs;
};
